...
def crop_helper(mesh_index, nViews, spreadViews, pLevel, input_dir, output_dir, mask_dir, debug_view, infmt, path):
    ...
    LoLo_point_indices = utils.crop_byVisibility(mesh, nViews, spreadViews, pLevel, mask, debug_view)
    ...
    if len(LoLo_point_indices) > 0:
        for i in range(0, len(LoLo_point_indices)):
            ...
            croppedM = utils.remove_points(mesh_cached,  list(set(vIDx) - set(LoLo_point_indices[i])))
            croppedM.save(str(out_path))
...
def do_crop_one(args):
    ...
    if args.nViews > 4:
        raise ValueError("--nViews needs to be set (ONLY integer Value) & can be Maximum 4")
    
    if args.spreadViews > 12:
        raise ValueError("--spreadViews needs to be set (ONLY integer Value) & can be Maximum 12")
    ...
    start = time.time()
    crop_helper([0], nViews, spreadViews, pLevel, input_dir, output_dir, mask_dir, debug_view, infmt, inputFile)
...
def do_crop_dir(args):
    ...
    """
    if args.pLevel > 2:
        raise ValueError("--cropLevel needs to be set & can be Maximum 2")
    ...
    start = time.time()
    if n_workers > 1:
        per_mesh_Crop = partial(crop_helper, range(n_meshes), nViews, spreadViews, pLevel, input_dir, output_dir, mask_dir, debug_view, infmt,)
        pool = Pool(processes=n_workers)
        pool.map(per_mesh_Crop, mesh_paths)
    else:
        for f in mesh_paths:
            crop_helper(range(n_meshes), nViews, spreadViews, pLevel, input_dir, output_dir, mask_dir, debug_view, infmt, f)
...
def _parse_args():
    ...
    parser_crop_dir.add_argument("--mask_dir", type=pathlib.Path,
    ...
    parser_crop_dir.add_argument("--nViews", type=int, default=2,
    ...
    parser_crop_dir.add_argument("--spreadViews", type=int, default=6,
        help="More spread of views means more visibility based Coverage",
    )
    parser_crop_dir.add_argument("--pLevel", type=int, default=3,
        help="Desired Level (maximum 5) of partiality of data",
    )
    parser_crop_dir.add_argument( "--seed", type=int, default=None,
        help="Initial state for the pseudo random number generator."
             " If not set, the initial state is not set explicitly.",
    )
    parser_crop_dir.add_argument("--nWorkers", type=int, default=1,
        help="Number of parallel processes. By default, the number of"
             " available processors.",
    )
    parser_crop_dir.add_argument("--infmt", required=False, choices=(
        ['.obj', '.npz']), default='.npz', help="Format of cropped mesh. Default is '.npz'")
    parser_crop_dir.add_argument("--debug_view", type=bool, default=False,
    ...
    parser_crop_one.add_argument("--mask_dir", type=pathlib.Path,
        help=" (optional) Directory tree with the masks (.npy). If defined,"
             " the partial data is created only on the non-masked faces of the"
             " meshes. (Only valid for challenge 1.)",
    )
    parser_crop_one.add_argument("--nViews", type=int, default=1,
        help="Number of views equivalent to no. of Cropped Meshes"
             " If nViews = 4 (default), the shape is saved as"
             " '<mesh_name>-partial.npz'."
             " If n > 4, the shapes are saved as '<mesh_name>-partial-XY.npz'"
    ...
    parser_crop_one.add_argument("--spreadViews", type=int, default=2,
        help="More spread of views means more visibility based Coverage",
    )
    parser_crop_one.add_argument("--pLevel", type=int, default=3,
        help="Desired Level (maximum 5) of partiality of data",
    )
    parser_crop_one.add_argument("--debug_view", type=bool, default=False,
                                 help="True To see the output of Crop",
    )
    parser_crop_one.add_argument("--infmt", required=False, choices=(
        ['.obj', '.npz']), default='.npz', help="Format of cropped mesh. Default is '.npz'")
