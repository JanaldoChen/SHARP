...
def crop_byVisibility(inputMesh, nViews=2, spreadViews=4, pLevel = 1, mask_faces=None, debug_view=False):
    def toO3DMesh(m):
    ...
    def get_viewPts(m, numViews, debug_view):
        ...
        for i in range(numViews):
            ...
            radius = math.sqrt(pow(abs(maxV - minV), 2) - pow(y, 2)) * 0.55 # radius at y
    ...
    def get_views(viewPoints, debug_view):
        ...
        for i in range(viewPoints.shape[0]):
            ...
            latitude = math.atan2(viewPoints[i, 2], math.sqrt(viewPoints[i, 0] ** 2 + viewPoints[i, 1] ** 2))
    ...
    def do_crop(meshIn, mask_faces, nViews, spreadViews, pLevel, debug_view):
        ...
        spts, sview_Rs, sview_ts = get_views(get_viewPts(mesh, nViews * 4, debug_view), debug_view)    # <----- sparse views
        dpts, dview_Rs, dview_ts = get_views(get_viewPts(mesh, nViews * 20, debug_view), debug_view)   # <----- dense views
        ...
        for sv in spts:
            ...
            if pLevel == 1: 
                if fA > 0.65 and fA < 0.95:
                    cm.paint_uniform_color([random.uniform(0.0, 0.8), random.uniform(0.5, 1.0), random.uniform(0.0, 0.5)])
                    if debug_view == True:
